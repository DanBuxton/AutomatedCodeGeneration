    ...
    public static async Task<IOException> CreateSystem(SystemInfo systemInfo, CancellationToken cancellationToken)
    {
        try
        {
                ...
                Relations = new()
            {
                new()
                {
                    RelationType = ClassRelationType.Inheritance,
                    Target = c1
                }
            }
            };

            model.Classes.Add(c1);
            model.Classes.Add(c2);

            model.Id = Guid.Parse("0a866869-1bc1-4ac6-c627-08d91fb3b958");

            if (await GetSystem(model.Id) is not null)
            {
                await AddSystem(model);
            }
#endif
            ...
            var isOk = await mgr.OutputFiles(outputHandler, cancellationToken);
